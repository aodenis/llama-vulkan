...
vk::Event llava_layer::execute(llava_context *ctx, vk::Event event) {
    ...
    vk::Event evtQ = ctx->matmul(ctx->current_Q, attention_wq, ctx->current_thought_sublayer, {evt_norm}); // maybe ok
    vk::Event evtK = ctx->matmul(ctx->current_K, attention_wk, ctx->current_thought_sublayer, {evt_norm}); // maybe ok
    vk::Event evtRoPEQ = ctx->rope(ctx->current_Q, {evtQ}); // maybe ok
    vk::Event evtRoPEK = ctx->rope(ctx->current_K, {evtK}); // maybe ok

    vk::Event evtV = ctx->matmul(ctx->current_V, attention_wv, ctx->current_thought_sublayer, {evt_norm}); // maybe ok
    vk::Event evtKc = ctx->kv_copy(k_cache, ctx->current_K, {evtRoPEK}); // ok
    vk::Event evtVc = ctx->kv_copy(v_cache, ctx->current_V, {evtV}); // ok
    vk::Event evtSA = ctx->multi_head_attention(ctx->attn_result, k_cache, ctx->current_Q, {evtRoPEQ, evtKc});
    vk::Event evtSoftmax = ctx->inplace_softmax(ctx->attn_result, {evtSA}); // ok
    ...
    vk::Event evt_w1 = ctx->matmul(ctx->properties_mask, feed_forward_w1, ctx->current_thought_sublayer, {evt_norm_ff});
    vk::Event evt_silu = ctx->silu(ctx->properties_mask, {evt_w1});
    vk::Event evt_w3 = ctx->matmul(ctx->properties_associated_values, feed_forward_w3, ctx->current_thought_sublayer, {evt_norm_ff});
    vk::Event evt_w13 = ctx->row_wise_multiply(ctx->properties_associated_values, ctx->properties_mask, {evt_silu, evt_w3});
    vk::Event evt_w2 = ctx->matmul(ctx->current_thought_sublayer, feed_forward_w2, ctx->properties_associated_values, {evt_w13});
